/* ============================================================
   RENDER-DARAZ.JS  v1.0
   Daraz Product Renderer

   WHAT THIS FILE DOES:
   ────────────────────────────────────────────────────────────
   1. COLLECTS all Daraz products loaded by the per-category
      files (Women.js … Flowers.js) from window.DarazProducts.

   2. EXPOSES per-category window vars that affiliate-main.js
      reads (DRZ_Women, DRZ_Men … DRZ_Crafts). This bridges
      the category files → affiliate-main pipeline.

   3. RENDERS a standalone Daraz product grid on any page
   ...
   4. PROVIDES a platform filter bar, sort dropdown, category
      ...
      and quick-view — all Daraz-specific.

   LOAD ORDER (before </body>):
   ──────────────────────────────
     <!-- Per-category files first (push into window.DarazProducts) -->
     <script src="js/daraz/Women.js"></script>
     <script src="js/daraz/Men.js"></script>
     <script src="js/daraz/Kids.js"></script>
     <script src="js/daraz/Shoes.js"></script>
     <script src="js/daraz/Bags.js"></script>
     <script src="js/daraz/Jewelry.js"></script>
     <script src="js/daraz/Watches.js"></script>
     <script src="js/daraz/Beauty.js"></script>
     <script src="js/daraz/Pets.js"></script>
     <script src="js/daraz/Toys.js"></script>
     <script src="js/daraz/Electronics.js"></script>
     <script src="js/daraz/Crafts.js"></script>
     <script src="js/daraz/Home.js"></script>
     <script src="js/daraz/Accessories.js"></script>
     <script src="js/daraz/Flowers.js"></script>
     <!-- Then this file -->
     <script src="js/render-daraz.js"></script>
     <!-- Then affiliate-main.js (reads DRZ_ vars we expose) -->
...
(function () {
  ...
  var PLATFORM    = 'Daraz';
  var PLATFORM_LC = 'daraz';
  var MP_ICON     = 'fas fa-store';
  var AFF_PARAM   = 'aff_id';
  var AFF_VALUE   = 'MEOW123';
  var AFF_DOMAIN  = 'daraz.pk';
  var BRAND_COLOR = '#F85606';  /* Daraz red-orange — used in platform badge */

  var ITEMS_PER_PAGE = 12;
  var WISHLIST_KEY   = 'meowmeow-wishlist';
  var GRID_ID        = 'darazGrid';   /* preferred; falls back to productGrid */

  /* ──────────────────────────────────────────────────
     COLLECT ALL DARAZ PRODUCTS
     The per-category files (Women.js, Men.js …) all push
     into window.DarazProducts via the push() pattern.
     We read that single array here.
  ────────────────────────────────────────────────── */

  var ALL = window.DarazProducts || [];

  if (ALL.length === 0) {
    console.warn('[render-daraz] No products found. Load Daraz category files before render-daraz.js.');
  }

  /* ──────────────────────────────────────────────────
     EXPOSE PER-CATEGORY VARS FOR affiliate-main.js
     affiliate-main.js reads window.DRZ_Women, window.DRZ_Men …
  ...
  var CATEGORY_MAP = {
    ...
    DRZ_Pets:        'pets',
    DRZ_Toys:        'toys',
    DRZ_Electronics: 'electronics',
    DRZ_Crafts:      'crafts',
    DRZ_Home:        'home',
    DRZ_Accessories: 'accessories',
    DRZ_Flowers:     'flowers'
  };
      ...
      '<div class="product-info">' +
        ...
        '<div class="product-actions">' +
          ...
          '<a href="' + esc(url) + '" class="btn btn-outline btn-block btn-sm"' +
            ...
            '<i class="fas fa-external-link-alt"></i> Buy on Daraz' +
          '</a>' +
        '</div>' +
      '</div>' +
    '</div>';
  ...
  function getGrid() {
    return document.getElementById(GRID_ID) ||
           document.querySelector('#productGrid[data-platform="daraz"]') ||
  ...
  function render(append) {
    ...
    var wlBtns = grid.querySelectorAll('.wishlist-toggle:not([data-drz-wl])');
    for (var i = 0; i < wlBtns.length; i++) {
      wlBtns[i]._meowBound    = true;
      wlBtns[i].dataset.drzWl = '1';
    }

    /* Stagger entrance animation */
    var newCards = grid.querySelectorAll('.product-card:not([data-drz-in])');
  ...
  function bindGridEvents() {
    ...
    /* WISHLIST */
    if (!grid._drz_wl_bound) {
      grid._drz_wl_bound = true;
    ...
    if (!grid._drz_cart_bound) {
      grid._drz_cart_bound = true;
      grid.addEventListener('click', function (e) {
    ...
    if (!grid._drz_qv_bound) {
      grid._drz_qv_bound = true;
      grid.addEventListener('click', function (e) {
          ...
          '<div style="padding:26px 22px;display:flex;flex-direction:column;gap:12px;overflow-y:auto;max-height:90vh;">' +
            ...
            '<div style="display:flex;flex-direction:column;gap:8px;margin-top:auto;padding-top:8px;">' +
              ...
              '<p style="font-size:.7rem;color:var(--text-tertiary);text-align:center;margin:0;">' +
                '<i class="fas fa-shield-alt"></i> Redirects to official Daraz listing' +
              '</p>' +
            '</div>' +
          '</div>' +
        '</div>' +
  ...
  function updateCount(grid) {
    ...
    el.textContent = filtered.length > 0
      ? 'Showing ' + show + ' of ' + filtered.length + ' Daraz products'
  ...
  function injectPlatformBadge() {
    if (document.getElementById('drzPlatformBadge')) return;
    var grid = getGrid();
    if (!grid) return;
    var badge = document.createElement('div');
    badge.id = 'drzPlatformBadge';
    badge.style.cssText =
      'display:inline-flex;align-items:center;gap:8px;' +
      'background:#F85606;color:#fff;' +
      'padding:6px 16px;border-radius:var(--radius-full);' +
      'font-size:.85rem;font-weight:700;margin-bottom:16px;';
    badge.innerHTML = '<i class="fas fa-store"></i> Daraz Deals';
  ...
  function injectSort() {
    if (document.getElementById('drzSort')) return;
    var wrap = document.createElement('div');
    wrap.style.cssText = 'display:flex;align-items:center;gap:8px;flex-shrink:0;';
    wrap.innerHTML =
      '<label for="drzSort" style="font-size:.82rem;color:var(--text-secondary);font-weight:500;white-space:nowrap;">Sort:</label>' +
      '<select id="drzSort" style="padding:8px 12px;background:var(--bg-secondary);border:2px solid var(--border);' +
    ...
    var sel = document.getElementById('drzSort');
    if (sel) {
  ...
  /* ──────────────────────────────────────────────────
     CATEGORY FILTER TABS (Daraz-specific)
  ────────────────────────────────────────────────── */

  function injectCategoryFilter() {
    if (document.getElementById('drzCatBar')) return;
    var grid = getGrid();
    if (!grid) return;
    ...
    bar.innerHTML = cats.map(function (c) {
      return '<button class="filter-btn' + (c.val === 'all' ? ' active' : '') + '"' +
             ' data-drz-cat="' + c.val + '">' + c.label + '</button>';
    }).join('');

    grid.insertAdjacentElement('beforebegin', bar);

    bar.addEventListener('click', function (e) {
      var btn = e.target.closest('[data-drz-cat]');
      if (!btn) return;
      bar.querySelectorAll('[data-drz-cat]').forEach(function (b) { b.classList.remove('active'); });
      btn.classList.add('active');
      activeCat = btn.dataset.drzCat;
      applyFilters();
      render(false);
    });
  }
  ...
  function bindSearch() {
    ...
    if (!input || input._drz_search_bound) return;
    input._drz_search_bound = true;
  ...
  function readUrlParam() {
    try {
      ...
      if (cat) {
        ...
        var btns = document.querySelectorAll('[data-drz-cat]');
        btns.forEach(function (b) {
          b.classList.toggle('active', b.dataset.drzCat === activeCat);
        });
      }
    } catch (e) {}
  }
  ...
  /* ──────────────────────────────────────────────────
     INIT — only runs if a Daraz grid target exists
  ────────────────────────────────────────────────── */
  ...
  function init() {
    ...
    console.log('[render-daraz] ✅ ' + ALL.length + ' Daraz products loaded across 15 categories');
  }
