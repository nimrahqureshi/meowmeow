...
/* ============================================================
   ...
   WHAT THIS FILE DOES:
   ────────────────────────────────────────────────────────────
   1. COLLECTS all Amazon products loaded by the per-category
      files (Women.js … Flowers.js) from window.AmazonProducts.

   2. EXPOSES per-category window vars that affiliate-main.js
      reads (AMZ_Women, AMZ_Men … AMZ_Crafts). This bridges
      the category files → affiliate-main pipeline.

   3. RENDERS a standalone Amazon product grid on any page
   ...
   4. PROVIDES a platform filter bar, sort dropdown, category
      ...
      and quick-view — all Amazon-specific.

   LOAD ORDER (before </body>):
   ──────────────────────────────
     <!-- Per-category files first (push into window.AmazonProducts) -->
     <script src="js/amazon/Women.js"></script>
     <script src="js/amazon/Men.js"></script>
     <script src="js/amazon/Kids.js"></script>
     <script src="js/amazon/Shoes.js"></script>
     <script src="js/amazon/Bags.js"></script>
     <script src="js/amazon/Jewelry.js"></script>
     <script src="js/amazon/Watches.js"></script>
     <script src="js/amazon/Beauty.js"></script>
     <script src="js/amazon/Pets.js"></script>
     <script src="js/amazon/Toys.js"></script>
     <script src="js/amazon/Electronics.js"></script>
     <script src="js/amazon/Crafts.js"></script>
     <script src="js/amazon/Home.js"></script>
     <script src="js/amazon/Accessories.js"></script>
     <script src="js/amazon/Flowers.js"></script>
     <!-- Then this file -->
     <script src="js/render-amazon.js"></script>
     <!-- Then affiliate-main.js (reads AMZ_ vars we expose) -->
...
(function () {
  ...
  var PLATFORM    = 'Amazon';
  var PLATFORM_LC = 'amazon';
  var MP_ICON     = 'fab fa-amazon';
  var AFF_PARAM   = 'tag';
  var AFF_VALUE   = 'meowmeow-21';
  var AFF_DOMAIN  = 'amazon.com';
  var BRAND_COLOR = '#FF9900';  /* Amazon orange — used in platform badge */

  var ITEMS_PER_PAGE = 12;
  var WISHLIST_KEY   = 'meowmeow-wishlist';
  var GRID_ID        = 'amazonGrid';   /* preferred; falls back to productGrid */

  /* ──────────────────────────────────────────────────
     COLLECT ALL AMAZON PRODUCTS
     The per-category files (Women.js, Men.js …) all push
     into window.AmazonProducts via the push() pattern.
     We read that single array here.
  ────────────────────────────────────────────────── */

  var ALL = window.AmazonProducts || [];

  if (ALL.length === 0) {
    console.warn('[render-amazon] No products found. Load Amazon category files before render-amazon.js.');
  }

  /* ──────────────────────────────────────────────────
     EXPOSE PER-CATEGORY VARS FOR affiliate-main.js
     affiliate-main.js reads window.AMZ_Women, window.AMZ_Men …
  ...
  var CATEGORY_MAP = {
    ...
    AMZ_Pets:        'pets',
    AMZ_Toys:        'toys',
    AMZ_Electronics: 'electronics',
    AMZ_Crafts:      'crafts',
    AMZ_Home:        'home',
    AMZ_Accessories: 'accessories',
    AMZ_Flowers:     'flowers'
  };
  ...
  function getGrid() {
    return document.getElementById(GRID_ID) ||
           document.querySelector('#productGrid[data-platform="amazon"]') ||
  ...
  function render(append) {
    ...
    var wlBtns = grid.querySelectorAll('.wishlist-toggle:not([data-azr-wl])');
    for (var i = 0; i < wlBtns.length; i++) {
      wlBtns[i]._meowBound    = true;
      wlBtns[i].dataset.azrWl = '1';
    }

    /* Stagger entrance animation */
    var newCards = grid.querySelectorAll('.product-card:not([data-azr-in])');
  ...
  function bindGridEvents() {
    ...
    if (!grid._azrCartBound) {
      grid._azrCartBound = true;
      grid.addEventListener('click', function (e) {
          ...
          '<div style="padding:26px 22px;display:flex;flex-direction:column;gap:12px;overflow-y:auto;max-height:90vh;">' +
            ...
            '<div style="display:flex;flex-direction:column;gap:8px;margin-top:auto;padding-top:8px;">' +
              ...
              '<p style="font-size:.7rem;color:var(--text-tertiary);text-align:center;margin:0;">' +
                '<i class="fas fa-shield-alt"></i> Redirects to official Amazon listing' +
              '</p>' +
            '</div>' +
          '</div>' +
        '</div>' +
  ...
  function updateCount(grid) {
    ...
    el.textContent = filtered.length > 0
      ? 'Showing ' + show + ' of ' + filtered.length + ' Amazon products'
  ...
  function injectPlatformBadge() {
    if (document.getElementById('azrPlatformBadge')) return;
    var grid = getGrid();
    if (!grid) return;
    var badge = document.createElement('div');
    badge.id = 'azrPlatformBadge';
    badge.style.cssText =
      'display:inline-flex;align-items:center;gap:8px;' +
      'background:#FF9900;color:#111;' +
      'padding:6px 16px;border-radius:var(--radius-full);' +
      'font-size:.85rem;font-weight:700;margin-bottom:16px;';
    badge.innerHTML = '<i class="fab fa-amazon"></i> Amazon Deals';
  ...
  function injectSort() {
    if (document.getElementById('azrSort')) return;
    var wrap = document.createElement('div');
    wrap.style.cssText = 'display:flex;align-items:center;gap:8px;flex-shrink:0;';
    wrap.innerHTML =
      '<label for="azrSort" style="font-size:.82rem;color:var(--text-secondary);font-weight:500;white-space:nowrap;">Sort:</label>' +
      '<select id="azrSort" style="padding:8px 12px;background:var(--bg-secondary);border:2px solid var(--border);' +
    ...
    var sel = document.getElementById('azrSort');
    if (sel) {
  ...
  /* ──────────────────────────────────────────────────
     CATEGORY FILTER TABS (Amazon-specific)
  ────────────────────────────────────────────────── */

  function injectCategoryFilter() {
    if (document.getElementById('azrCatBar')) return;
    var grid = getGrid();
    if (!grid) return;
    ...
    bar.innerHTML = cats.map(function (c) {
      return '<button class="filter-btn' + (c.val === 'all' ? ' active' : '') + '"' +
             ' data-azr-cat="' + c.val + '">' + c.label + '</button>';
    }).join('');

    grid.insertAdjacentElement('beforebegin', bar);

    bar.addEventListener('click', function (e) {
      var btn = e.target.closest('[data-azr-cat]');
      if (!btn) return;
      bar.querySelectorAll('[data-azr-cat]').forEach(function (b) { b.classList.remove('active'); });
      btn.classList.add('active');
      activeCat = btn.dataset.azrCat;
      applyFilters();
      render(false);
    });
  }
  ...
  function bindSearch() {
    ...
    if (!input || input._azrSearchBound) return;
    input._azrSearchBound = true;
  ...
  function readUrlParam() {
    try {
      ...
      if (cat) {
        ...
        var btns = document.querySelectorAll('[data-azr-cat]');
        btns.forEach(function (b) {
          b.classList.toggle('active', b.dataset.azrCat === activeCat);
        });
      }
    } catch (e) {}
  }
  ...
  /* ──────────────────────────────────────────────────
     INIT — only runs if an Amazon grid target exists
  ────────────────────────────────────────────────── */
  ...
  function init() {
    ...
    console.log('[render-amazon] ✅ ' + ALL.length + ' Amazon products loaded across 15 categories');
  }
