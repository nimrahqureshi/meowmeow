/* ============================================================
   ...
   WHAT THIS FILE DOES:
   ────────────────────────────────────────────────────────────
   1. COLLECTS all AliExpress products loaded by the per-category
      files (Women.js … Flowers.js) from window.AliExpressProducts.

   2. EXPOSES per-category window vars that affiliate-main.js
      reads (AEX_Women, AEX_Men … AEX_Crafts). This bridges
      the category files → affiliate-main pipeline.

   3. RENDERS a standalone AliExpress product grid on any page
   ...
   4. PROVIDES a platform filter bar, sort dropdown, category
      ...
      and quick-view — all AliExpress-specific.

   LOAD ORDER (before </body>):
   ──────────────────────────────
     <!-- Per-category files first (push into window.AliExpressProducts) -->
     <script src="js/aliexpress/Women.js"></script>
     <script src="js/aliexpress/Men.js"></script>
     <script src="js/aliexpress/Kids.js"></script>
     <script src="js/aliexpress/Shoes.js"></script>
     <script src="js/aliexpress/Bags.js"></script>
     <script src="js/aliexpress/Jewelry.js"></script>
     <script src="js/aliexpress/Watches.js"></script>
     <script src="js/aliexpress/Beauty.js"></script>
     <script src="js/aliexpress/Pets.js"></script>
     <script src="js/aliexpress/Toys.js"></script>
     <script src="js/aliexpress/Electronics.js"></script>
     <script src="js/aliexpress/Crafts.js"></script>
     <script src="js/aliexpress/Home.js"></script>
     <script src="js/aliexpress/Accessories.js"></script>
     <script src="js/aliexpress/Flowers.js"></script>
     <!-- Then this file -->
     <script src="js/render-aliexpress.js"></script>
     <!-- Then affiliate-main.js (reads AEX_ vars we expose) -->
...
(function () {
  ...
  var PLATFORM    = 'AliExpress';
  var PLATFORM_LC = 'aliexpress';
  var MP_ICON     = 'fas fa-globe';
  var AFF_PARAM   = 'aff_platform';
  var AFF_VALUE   = 'meowmeow_site';
  var AFF_DOMAIN  = 'aliexpress.com';
  var BRAND_COLOR = '#FF6A00';  /* AliExpress orange — used in platform badge */
  ...
  var ALL = window.AliExpressProducts || [];

  if (ALL.length === 0) {
    console.warn('[render-aliexpress] No products found. Load AliExpress category files before render-aliexpress.js.');
  }

  /* ──────────────────────────────────────────────────
     EXPOSE PER-CATEGORY VARS FOR affiliate-main.js
     affiliate-main.js reads window.AEX_Women, window.AEX_Men …
  ...
  var CATEGORY_MAP = {
    ...
    AEX_Pets:        'pets',
    AEX_Toys:        'toys',
    AEX_Electronics: 'electronics',
    AEX_Crafts:      'crafts',
    AEX_Home:        'home',
    AEX_Accessories: 'accessories',
    AEX_Flowers:     'flowers'
  };
      ...
      '<div class="product-info">' +
        ...
        '<div class="product-actions">' +
          ...
          '<a href="' + esc(url) + '" class="btn btn-outline btn-block btn-sm"' +
            ...
            '<i class="fas fa-external-link-alt"></i> Buy on AliExpress' +
          '</a>' +
        '</div>' +
      '</div>' +
    '</div>';
  ...
  function getGrid() {
    return document.getElementById(GRID_ID) ||
           document.querySelector('#productGrid[data-platform="aliexpress"]') ||
  ...
  function render(append) {
    ...
    var wlBtns = grid.querySelectorAll('.wishlist-toggle:not([data-aex-wl])');
    for (var i = 0; i < wlBtns.length; i++) {
      wlBtns[i]._meowBound    = true;
      wlBtns[i].dataset.aexWl = '1';
    }

    /* Stagger entrance animation */
    var newCards = grid.querySelectorAll('.product-card:not([data-aex-in])');
  ...
  function bindGridEvents() {
    ...
    /* WISHLIST */
    if (!grid._aex_wl_bound) {
      grid._aex_wl_bound = true;
    ...
    if (!grid._aex_cart_bound) {
      grid._aex_cart_bound = true;
      grid.addEventListener('click', function (e) {
    ...
    if (!grid._aex_qv_bound) {
      grid._aex_qv_bound = true;
      grid.addEventListener('click', function (e) {
          ...
          '<div style="padding:26px 22px;display:flex;flex-direction:column;gap:12px;overflow-y:auto;max-height:90vh;">' +
            ...
            '<div style="display:flex;flex-direction:column;gap:8px;margin-top:auto;padding-top:8px;">' +
              ...
              '<p style="font-size:.7rem;color:var(--text-tertiary);text-align:center;margin:0;">' +
                '<i class="fas fa-shield-alt"></i> Redirects to official AliExpress listing' +
              '</p>' +
            '</div>' +
          '</div>' +
        '</div>' +
  ...
  function updateCount(grid) {
    ...
    el.textContent = filtered.length > 0
      ? 'Showing ' + show + ' of ' + filtered.length + ' AliExpress products'
  ...
  function injectPlatformBadge() {
    if (document.getElementById('aexPlatformBadge')) return;
    var grid = getGrid();
    if (!grid) return;
    var badge = document.createElement('div');
    badge.id = 'aexPlatformBadge';
    badge.style.cssText =
      'display:inline-flex;align-items:center;gap:8px;' +
      'background:#FF6A00;color:#fff;' +
      'padding:6px 16px;border-radius:var(--radius-full);' +
      'font-size:.85rem;font-weight:700;margin-bottom:16px;';
    badge.innerHTML = '<i class="fas fa-globe"></i> AliExpress Deals';
  ...
  function injectSort() {
    if (document.getElementById('aexSort')) return;
    var wrap = document.createElement('div');
    wrap.style.cssText = 'display:flex;align-items:center;gap:8px;flex-shrink:0;';
    wrap.innerHTML =
      '<label for="aexSort" style="font-size:.82rem;color:var(--text-secondary);font-weight:500;white-space:nowrap;">Sort:</label>' +
      '<select id="aexSort" style="padding:8px 12px;background:var(--bg-secondary);border:2px solid var(--border);' +
    ...
    var sel = document.getElementById('aexSort');
    if (sel) {
  ...
  /* ──────────────────────────────────────────────────
     CATEGORY FILTER TABS (AliExpress-specific)
  ────────────────────────────────────────────────── */

  function injectCategoryFilter() {
    if (document.getElementById('aexCatBar')) return;
    var grid = getGrid();
    if (!grid) return;
    ...
    bar.innerHTML = cats.map(function (c) {
      return '<button class="filter-btn' + (c.val === 'all' ? ' active' : '') + '"' +
             ' data-aex-cat="' + c.val + '">' + c.label + '</button>';
    }).join('');

    grid.insertAdjacentElement('beforebegin', bar);

    bar.addEventListener('click', function (e) {
      var btn = e.target.closest('[data-aex-cat]');
      if (!btn) return;
      bar.querySelectorAll('[data-aex-cat]').forEach(function (b) { b.classList.remove('active'); });
      btn.classList.add('active');
      activeCat = btn.dataset.aexCat;
      applyFilters();
      render(false);
    });
  }
  ...
  function bindSearch() {
    ...
    if (!input || input._aex_search_bound) return;
    input._aex_search_bound = true;
  ...
  function readUrlParam() {
    try {
      ...
      if (cat) {
        ...
        var btns = document.querySelectorAll('[data-aex-cat]');
        btns.forEach(function (b) {
          b.classList.toggle('active', b.dataset.aexCat === activeCat);
        });
      }
    } catch (e) {}
  }
  ...
  /* ──────────────────────────────────────────────────
     INIT — only runs if a AliExpress grid target exists
  ────────────────────────────────────────────────── */
  ...
  function init() {
    ...
    console.log('[render-aliexpress] ✅ ' + ALL.length + ' AliExpress products loaded across 15 categories');
  }
